...
# Added tracking for how often users call the bot, 
# Fixed the /converse bug with bot output, meaning the output should be high quality regardless of where or when you use it.
# The bot now has autocorrect on emoji, so it can now output emoji with much higher accuracy.
...
def recursive_generate(prompt, num_quotes, uname='NoName'):
    ...
    for i in range(num_quotes):
        ...
        for content in response.choices:
            ...
            prompt += content.text + '\n'
        output += message_str
    return fix_emoji(output)

# Get an emoji by its name. Returns empty string if no such emoji exists.
...
# Fix broken emoji in a given string
def fix_emoji(string):
    emoji_strings = re.findall('<:[A-z]+:\d+>?',string)
    for emoji_string in emoji_strings:
        emoji_name = emoji_string.split(':')[1]
        emoji = get_emoji_by_name(emoji_name)
        string = string.replace(emoji_string, emoji)
    return string
